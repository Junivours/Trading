...
def last_value(x):
    """Helper function to safely get the last value from arrays or return scalar values"""
            ...
            for key, values in indicators.items():
                if values is not None and len(values) > 0:
                    ...
                    if isinstance(last_val, (int, float, np.number)) and not (np.isnan(last_val) or np.isinf(last_val)):
                        current_values[f'current_{key}'] = float(last_val)
                    else:
    ...
    def detect_all_patterns(ohlc_data):
        try:
           ...
           # Single Candle Patterns
            ...
            patterns['inverted_hammer'] = last_value(talib.CDLINVERTEDHAMMER(open_prices, high_prices, low_prices, close_prices)) != 0
            patterns['shooting_star'] = last_value(talib.CDLSHOOTINGSTAR(open_prices, high_prices, low_prices, close_prices)) != 0
            patterns['hanging_man'] = last_value(talib.CDLHANGINGMAN(open_prices, high_prices, low_prices, close_prices)) != 0
            ...
            patterns['harami_cross'] = last_value(talib.CDLHARAMICROSS(open_prices, high_prices, low_prices, close_prices)) != 0
            ...
            patterns['tweezer_tops'] = last_value(talib.CDLINNECK(open_prices, high_prices, low_prices, close_prices)) != 0
            ...
            patterns['morning_doji_star'] = last_value(talib.CDLMORNINGDOJISTAR(open_prices, high_prices, low_prices, close_prices)) != 0
            patterns['evening_doji_star'] = last_value(talib.CDLEVENINGDOJISTAR(open_prices, high_prices, low_prices, close_prices)) != 0
            ...
            patterns['three_inside_up'] = last_value(talib.CDL3INSIDE(open_prices, high_prices, low_prices, close_prices)) > 0
            patterns['three_inside_down'] = last_value(talib.CDL3INSIDE(open_prices, high_prices, low_prices, close_prices)) < 0
            patterns['three_outside_up'] = last_value(talib.CDL3OUTSIDE(open_prices, high_prices, low_prices, close_prices)) > 0
            patterns['three_outside_down'] = last_value(talib.CDL3OUTSIDE(open_prices, high_prices, low_prices, close_prices)) < 0
            
            # Advanced Patterns
            patterns['abandoned_baby_bullish'] = last_value(talib.CDLABANDONEDBABY(open_prices, high_prices, low_prices, close_prices)) > 0
            patterns['abandoned_baby_bearish'] = last_value(talib.CDLABANDONEDBABY(open_prices, high_prices, low_prices, close_prices)) < 0
            patterns['advance_block'] = last_value(talib.CDLADVANCEBLOCK(open_prices, high_prices, low_prices, close_prices)) != 0
            patterns['belt_hold'] = last_value(talib.CDLBELTHOLD(open_prices, high_prices, low_prices, close_prices)) != 0
            patterns['breakaway'] = last_value(talib.CDLBREAKAWAY(open_prices, high_prices, low_prices, close_prices)) != 0
            patterns['closing_marubozu'] = last_value(talib.CDLCLOSINGMARUBOZU(open_prices, high_prices, low_prices, close_prices)) != 0
            patterns['concealing_baby_swallow'] = last_value(talib.CDLCONCEALBABYSWALL(open_prices, high_prices, low_prices, close_prices)) != 0
            patterns['counterattack'] = last_value(talib.CDLCOUNTERATTACK(open_prices, high_prices, low_prices, close_prices)) != 0
            patterns['doji_star'] = last_value(talib.CDLDOJISTAR(open_prices, high_prices, low_prices, close_prices)) != 0
            patterns['gapping_up_side_white'] = last_value(talib.CDLGAPSIDESIDEWHITE(open_prices, high_prices, low_prices, close_prices)) != 0
            patterns['high_wave'] = last_value(talib.CDLHIGHWAVE(open_prices, high_prices, low_prices, close_prices)) != 0
            patterns['hikkake'] = last_value(talib.CDLHIKKAKE(open_prices, high_prices, low_prices, close_prices)) != 0
            patterns['modified_hikkake'] = last_value(talib.CDLHIKKAKEMOD(open_prices, high_prices, low_prices, close_prices)) != 0
            patterns['homing_pigeon'] = last_value(talib.CDLHOMINGPIGEON(open_prices, high_prices, low_prices, close_prices)) != 0
            patterns['identical_three_crows'] = last_value(talib.CDLIDENTICAL3CROWS(open_prices, high_prices, low_prices, close_prices)) != 0
            patterns['in_neck'] = last_value(talib.CDLINNECK(open_prices, high_prices, low_prices, close_prices)) != 0
            patterns['inverted_hammer'] = last_value(talib.CDLINVERTEDHAMMER(open_prices, high_prices, low_prices, close_prices)) != 0
            patterns['kicking'] = last_value(talib.CDLKICKING(open_prices, high_prices, low_prices, close_prices)) != 0
            patterns['kicking_by_length'] = last_value(talib.CDLKICKINGBYLENGTH(open_prices, high_prices, low_prices, close_prices)) != 0
            patterns['ladder_bottom'] = last_value(talib.CDLLADDERBOTTOM(open_prices, high_prices, low_prices, close_prices)) != 0
            patterns['long_legged_doji'] = last_value(talib.CDLLONGLEGGEDDOJI(open_prices, high_prices, low_prices, close_prices)) != 0
            patterns['long_line'] = last_value(talib.CDLLONGLINE(open_prices, high_prices, low_prices, close_prices)) != 0
            patterns['matching_low'] = last_value(talib.CDLMATCHINGLOW(open_prices, high_prices, low_prices, close_prices)) != 0
            patterns['mat_hold'] = last_value(talib.CDLMATHOLD(open_prices, high_prices, low_prices, close_prices)) != 0
            patterns['on_neck'] = last_value(talib.CDLONNECK(open_prices, high_prices, low_prices, close_prices)) != 0
            patterns['rickshaw_man'] = last_value(talib.CDLRICKSHAWMAN(open_prices, high_prices, low_prices, close_prices)) != 0
            patterns['rise_fall_three_methods'] = last_value(talib.CDLRISEFALL3METHODS(open_prices, high_prices, low_prices, close_prices)) != 0
            patterns['separating_lines'] = last_value(talib.CDLSEPARATINGLINES(open_prices, high_prices, low_prices, close_prices)) != 0
            patterns['short_line'] = last_value(talib.CDLSHORTLINE(open_prices, high_prices, low_prices, close_prices)) != 0
            patterns['stalled_pattern'] = last_value(talib.CDLSTALLEDPATTERN(open_prices, high_prices, low_prices, close_prices)) != 0
            patterns['stick_sandwich'] = last_value(talib.CDLSTICKSANDWICH(open_prices, high_prices, low_prices, close_prices)) != 0
            patterns['takuri'] = last_value(talib.CDLTAKURI(open_prices, high_prices, low_prices, close_prices)) != 0
            patterns['tasuki_gap'] = last_value(talib.CDLTASUKIGAP(open_prices, high_prices, low_prices, close_prices)) != 0
            patterns['thrusting'] = last_value(talib.CDLTHRUSTING(open_prices, high_prices, low_prices, close_prices)) != 0
            patterns['tristar'] = last_value(talib.CDLTRISTAR(open_prices, high_prices, low_prices, close_prices)) != 0
            patterns['unique_three_river'] = last_value(talib.CDLUNIQUE3RIVER(open_prices, high_prices, low_prices, close_prices)) != 0
            patterns['upside_gap_two_crows'] = last_value(talib.CDLUPSIDEGAP2CROWS(open_prices, high_prices, low_prices, close_prices)) != 0
            patterns['xside_gap_three_methods'] = last_value(talib.CDLXSIDEGAP3METHODS(open_prices, high_prices, low_prices, close_prices)) != 0
...
class AdvancedMLPredictor:
    ...
    def _extract_comprehensive_features(indicators, patterns, price_data, volume_data):
        ...
        features['bullish_pattern_count'] = 0
        features['bearish_pattern_count'] = 0
        features['pattern_strength'] = 0
        
        # Price features
        recent_prices = [p['close'] for p in price_data[-20:]]
        if len(recent_prices) > 0:
            features['price_trend'] = (last_value(recent_prices) - recent_prices[0]) / recent_prices[0]
            features['price_volatility'] = np.std(recent_prices) / np.mean(recent_prices)
            features['price_momentum'] = (last_value(recent_prices) - recent_prices[-5]) / recent_prices[-5] if len(recent_prices) >= 5 else 0
        
        # Volume features
        recent_volumes = volume_data[-10:] if len(volume_data) >= 10 else volume_data
        if len(recent_volumes) > 1:
            features['volume_trend'] = (last_value(recent_volumes) - recent_volumes[0]) / recent_volumes[0]
            features['volume_spike'] = last_value(recent_volumes) / np.mean(recent_volumes[:-1]) if len(recent_volumes) > 1 else 1
        ...
        features['bb_position'] = AdvancedMLPredictor._calculate_bb_position(indicators, last_value(recent_prices) if recent_prices else 0)
    ...
    def _calculate_bb_position(indicators, current_price):
        ...
        try:
            ...
            logger.error(f"Error calculating BB position: {str(e)}")
            return 0.5
...
class AdvancedMarketAnalyzer:
    ...
    def _generate_trading_signals(indicators, patterns, ml_predictions, price_data):
        """Generate trading signals based on indicators, patterns and ML predictions"""
        signals = []
        ...
        # Pattern Signals
        if patterns.get('hammer') or patterns.get('morning_star'):
            signals.append({
                'type': 'BUY',
                'source': 'BULLISH_PATTERN',
                'strength': 0.7,
                'description': 'Bullish candlestick pattern detected'
            })
        
        if patterns.get('shooting_star') or patterns.get('evening_star'):
            signals.append({
                'type': 'SELL',
                'source': 'BEARISH_PATTERN',
                'strength': 0.7,
                'description': 'Bearish candlestick pattern detected'
            })
        
        # ML Prediction Signals
        for timeframe, prediction in ml_predictions.items():
            if prediction.get('direction') == 'BUY' and prediction.get('confidence', 0) > 70:
                signals.append({
                    'type': 'BUY',
                    'source': f'ML_{timeframe.upper()}',
                    'strength': prediction.get('confidence', 0) / 100,
                    'description': f'ML {timeframe} prediction: {prediction.get("confidence", 0):.1f}% confidence'
                })
            elif prediction.get('direction') == 'SELL' and prediction.get('confidence', 0) > 70:
                signals.append({
                    'type': 'SELL',
                    'source': f'ML_{timeframe.upper()}',
                    'strength': prediction.get('confidence', 0) / 100,
    ...
    def _calculate_trading_kpis(indicators, patterns, price_data, volume_data):
        """Calculate key performance indicators for trading"""
        kpis = {}
        
        # Price KPIs
        if len(price_data) > 0:
            current_price = last_value(price_data)['close'] if isinstance(last_value(price_data), dict) else last_value([p['close'] for p in price_data])
            high_24h = max([p['high'] for p in price_data[-24:]] if len(price_data) >= 24 else [p['high'] for p in price_data])
            low_24h = min([p['low'] for p in price_data[-24:]] if len(price_data) >= 24 else [p['low'] for p in price_data])
            
            kpis['price_position'] = (current_price - low_24h) / (high_24h - low_24h) if high_24h != low_24h else 0.5
            kpis['price_change_24h'] = (current_price - price_data[-24]['close']) / price_data[-24]['close'] * 100 if len(price_data) >= 24 else 0
        
        # Volatility
        if len(price_data) >= 20:
            recent_prices = [p['close'] for p in price_data[-20:]]
            kpis['volatility'] = np.std(recent_prices) / np.mean(recent_prices) * 100
        else:
            kpis['volatility'] = 0
        
        # Volume KPIs
        if len(volume_data) > 0:
            current_volume = last_value(volume_data)
            avg_volume = np.mean(volume_data[-10:]) if len(volume_data) >= 10 else current_volume
            kpis['volume_ratio'] = current_volume / avg_volume if avg_volume > 0 else 1
        
        # Technical KPIs
        kpis['rsi_level'] = indicators.get('current_rsi_14', 50)
        kpis['trend_strength'] = AdvancedMarketAnalyzer._calculate_trend_strength_kpi(indicators)
        kpis['momentum'] = AdvancedMarketAnalyzer._calculate_momentum_kpi(indicators)
    ...
    def _calculate_sentiment_score(signals, kpis, ml_predictions):
        ...
        if buy_strength > sell_strength * 1.2:
            sentiment = 'BULLISH'
            confidence = min(95, 50 + buy_strength * 10)
        elif sell_strength > buy_strength * 1.2:
            sentiment = 'BEARISH'
            confidence = min(95, 50 + sell_strength * 10)
        else:
            sentiment = 'NEUTRAL'
            confidence = 50
        
        # Calculate trading score
        trading_score = (buy_strength - sell_strength) * 10
        trading_score = max(-10, min(10, trading_score))
        
        # Determine recommended action
        if trading_score > 3:
            action = 'STRONG_BUY'
        elif trading_score > 1:
            action = 'BUY'
    ...
    def _detect_market_state(indicators, price_data, volume_data):
        ...
        recent_prices = [p['close'] for p in price_data[-20:]]
        price_trend = (last_value(recent_prices) - recent_prices[0]) / recent_prices[0]
        volatility = np.std(recent_prices) / np.mean(recent_prices)
        
        # Volume analysis
        if len(volume_data) >= 10:
            recent_volumes = volume_data[-10:]
            volume_trend = (last_value(recent_volumes) - recent_volumes[0]) / recent_volumes[0]
        else:
            volume_trend = 0
        
        # Determine market state
        if abs(price_trend) < 0.02 and volatility < 0.03:
    ...
    @staticmethod
    def _calculate_trend_strength_kpi(indicators):
        """Calculate trend strength from multiple indicators"""
        ema_20 = indicators.get('current_ema_20', 0)
        ema_50 = indicators.get('current_ema_50', 0)
        sma_200 = indicators.get('current_sma_200', 0)
    ...
    @staticmethod
    def _calculate_momentum_kpi(indicators):
        """Calculate momentum from MACD and RSI"""
        macd = indicators.get('current_macd', 0)
        macd_signal = indicators.get('current_macd_signal', 0)
        rsi = indicators.get('current_rsi_14', 50)
    ...
    @staticmethod
    def _calculate_momentum_kpi(indicators):
        """Calculate momentum from RSI and MACD"""
        rsi = indicators.get('current_rsi_14', 50)
        macd = indicators.get('current_macd', 0)
        macd_signal = indicators.get('current_macd_signal', 0)
        ...
        return (rsi_momentum + abs(macd_momentum)) / 2
...
def analyze_symbol():
    try:
        ...
        # Convert OHLC data to proper format for analysis
        price_data = []
        volume_data = []
...
def get_ultimate_dashboard_html():
    ...
    <head>
        ...
        /* Hover Animations for KPI Cards - Simplified */
        .kpi-card {
        ...
        .gauge-fill {
            ...
            /* transition: stroke-dasharray 1s ease-in-out; Removed for performance */
        }
...
<body>
    ...
    <script>
                ...
                <div class="price-info">
                    <div class="price-main">
                        ...
                        <div class="price-change ${changeClass}">
                            ${changeIcon} ${data.price_change_24h >= 0 ? '+' : ''}${formatPercentage(data.price_change_24h, 2)}
                        </div>
                        <div class="market-state">
                            <div class="market-state-label">Marktzustand</div>
                            <div class="market-state-value">${data.market_analysis.market_state || 'ANALYSIEREND'}</div>
                        </div>
                    </div>
                ...
                <div style="display: grid; grid-template-columns: 1fr 2fr; gap: 1rem; align-items: center;">
                    <div class="trading-score-gauge">
                        <svg width="80" height="80" viewBox="0 0 80 80">
                            <circle cx="40" cy="40" r="35" class="gauge-bg"></circle>
                            <circle cx="40" cy="40" r="35" class="gauge-fill" 
                                    style="stroke-dasharray: ${(tradingScore / 100) * 220} 220; 
                                           stroke: ${getScoreColor(tradingScore)}"></circle>
                        </svg>
                        <div class="gauge-text">
                            <div style="font-size: 1.1rem; color: ${getScoreColor(tradingScore)}">${formatDecimal(tradingScore, 0)}</div>
                            <div style="font-size: 0.6rem; color: var(--text-secondary)">Score</div>
                        </div>
                    </div>
                    <div class="kpi-grid" style="margin: 0;">
                        <div class="kpi-card ${getTrendClass(kpis.trend_strength)}">
                            <div class="kpi-header">
                                ...
                                <div class="kpi-status status-${getTrendClass(kpis.trend_strength)}">${formatDecimal((kpis.trend_strength || 0) * 25, 0)}%</div>
                            </div>
                            <div class="kpi-value">${formatScore(kpis.trend_strength, 4)}</div>
                        </div>
                        <div class="kpi-card ${getVolatilityClass(kpis.volatility_20d)}">
                            <div class="kpi-header">
                                <div class="kpi-name">Volatilität</div>
                                <div class="kpi-status status-${getVolatilityClass(kpis.volatility_20d)}">${formatDecimal(kpis.volatility_20d || 0, 1)}%</div>
                            </div>
                        ...
                        <div class="kpi-card ${getMomentumClass(kpis.momentum_score)}">
                            <div class="kpi-header">
                                ...
                                <div class="kpi-status status-${getMomentumClass(kpis.momentum_score)}">${formatDecimal((kpis.momentum_score || 0) * 33, 0)}%</div>
                            </div>
                            <div class="kpi-value">${formatScore(kpis.momentum_score, 3)}</div>
                        </div>
                        <div class="kpi-card ${getEfficiencyClass(kpis.market_efficiency)}">
                            <div class="kpi-header">
                                <div class="kpi-name">Effizienz</div>
                                <div class="kpi-status status-${getEfficiencyClass(kpis.market_efficiency)}">${formatDecimal(kpis.market_efficiency || 0, 0)}%</div>
                            </div>
        ...
        function createTechnicalIndicatorsCard(indicators) {
            ...
            const oscillatorContent = `
                <div class="indicators-grid">
                    ...
                    ${createIndicatorItem('Williams %R', formatDecimal(indicators.williams_r, 2), getRSIStatus(100 + indicators.williams_r), 'Williams Percent Range')}
                </div>
            `;

            const trendContent = `
                <div class="indicators-grid">
                    ${createIndicatorItem('EMA 20', formatDecimal(indicators.ema_20, 2), getEMAStatus(indicators.ema_20, indicators.ema_50), 'Exponential Moving Average 20')}
                    ${createIndicatorItem('SMA 50', formatDecimal(indicators.sma_50, 2), getSMAStatus(indicators.sma_20, indicators.sma_50), 'Simple Moving Average 50')}
                    ${createIndicatorItem('ADX', formatDecimal(indicators.adx, 2), getADXStatus(indicators.adx), 'Average Directional Index')}
                    ${createIndicatorItem('Bollinger Upper', formatDecimal(indicators.bb_upper, 2), {status: 'Normal', color: 'var(--info)'}, 'Bollinger Band Upper')}
                </div>
            `;

            const momentumContent = `
                <div class="indicators-grid">
                    ${createIndicatorItem('MACD', formatDecimal(indicators.macd, 4), getMACDStatus(indicators.macd, indicators.macd_signal, indicators.macd_histogram), 'MACD Linie vs Signal')}
                    ${createIndicatorItem('MACD Signal', formatDecimal(indicators.macd_signal, 4), getMACDStatus(indicators.macd, indicators.macd_signal, indicators.macd_histogram), 'MACD Signal Line')}
                    ${createIndicatorItem('MACD Histogram', formatDecimal(indicators.macd_histogram, 4), getMACDStatus(indicators.macd, indicators.macd_signal, indicators.macd_histogram), 'MACD Histogram')}
                    ${createIndicatorItem('ATR', formatDecimal(indicators.atr, 2), {status: 'Normal', color: 'var(--info)'}, 'Average True Range')}
                </div>
            `;

            const volumeContent = `
                <div class="indicators-grid">
                    ${createIndicatorItem('OBV', formatDecimal(indicators.obv, 0), {status: 'Normal', color: 'var(--info)'}, 'On Balance Volume')}
                    ${createIndicatorItem('A/D Line', formatDecimal(indicators.ad, 0), {status: 'Normal', color: 'var(--info)'}, 'Accumulation/Distribution Line')}
                    ...
                    ${signals && signals.length > 0 ? signals.map(signal => `
                        <div class="signal-item" style="border-left-color: ${getSignalColor(signal.type)}">
                            ...
                            <div class="signal-meta">
                                <span>Stärke: ${formatDecimal(signal.strength, 0)}/10</span>
                                <span>Konfidenz: ${formatPercentage(signal.confidence, 0)}</span>
                    ...
                    <div style="text-align: center;">
                        ...
                        <div style="display: grid; grid-template-columns: repeat(2, 1fr); gap: 1rem;">
                            <div class="stat-item">
                                ...
                                <div class="stat-value">${formatPercentage(analysis.confidence, 1)}</div>
                            </div>
                            <div class="stat-item">
                                <div class="stat-label">Stärke</div>
                                <div class="stat-value">${formatScore(analysis.strength, 10)}</div>
                            </div>
                            <div class="stat-item">
                            ...
                            <div class="stat-item">
                                ...
                                <div class="stat-value">${formatDecimal(analysis.trading_score || 50, 0)}/100</div>
                            </div>
                        </div>
                    </div>
                    <div>
                        ...
                        <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(150px, 1fr)); gap: 0.8rem;">
                            ...
                            <div class="kpi-card" style="padding: 0.8rem;">
                                ...
                                <div class="kpi-value">${formatDecimal((kpis.bb_position || 0.5) * 100, 0)}%</div>
                            </div>
                            <div class="kpi-card" style="padding: 0.8rem;">
                            ...
                            <div class="kpi-card" style="padding: 0.8rem;">
                                ...
                                <div class="kpi-value" style="color: ${kpis.pattern_sentiment > 0 ? 'var(--success)' : kpis.pattern_sentiment < 0 ? 'var(--danger)' : 'var(--warning)'}">
                                    ${kpis.pattern_sentiment > 0 ? '+' : ''}${formatDecimal(kpis.pattern_sentiment || 0, 1)}
                                </div>
                            </div>
                        </div>
                    </div>
                            ...
                            ${analysis.signals.slice(0, 3).map((signal, index) => `
                                <div style="background: rgba(15, 15, 35, 0.3); border-radius: 8px; padding: 1rem; border-left: 3px solid ${getSignalColor(signal.type)};">
                                    ...
                                    <div style="font-size: 0.75rem; color: var(--text-secondary);">
                                        Stärke: ${formatDecimal(signal.strength, 0)}/10 | Konfidenz: ${formatPercentage(signal.confidence, 0)} | ${signal.timeframe}
                                    </div>
                                </div>
                            `).join('')}
                        </div>
        ...
        // Helper Funktionen für Status-Berechnung
        ...
        function getMACDStatus(macd, signal, histogram) {
            if (macd > signal && histogram > 0) return { status: 'Bullish', color: 'var(--success)' };
            if (macd < signal && histogram < 0) return { status: 'Bearish', color: 'var(--danger)' };
            return { status: 'Seitwärts', color: 'var(--warning)' };
        }

        function getEMAStatus(ema12, ema26) {
            if (ema12 > ema26) return { status: 'Aufwärts', color: 'var(--success)' };
            if (ema12 < ema26) return { status: 'Abwärts', color: 'var(--danger)' };
            return { status: 'Seitwärts', color: 'var(--warning)' };
        }
        ...
        function getBollingerStatus(price, upper, lower) {
            if (price >= upper) return { status: 'Oberes Band', color: 'var(--danger)' };
            if (price <= lower) return { status: 'Unteres Band', color: 'var(--success)' };
            return { status: 'Mittlerer Bereich', color: 'var(--warning)' };
        }
        ...
        function createIndicatorItem(name, value, status, description) {
            const statusClass = status.status === 'Bullish' || status.status === 'Aufwärts' || status.status === 'Überverkauft' || status.status === 'Hoch' ? 'bullish' :
                               status.status === 'Bearish' || status.status === 'Abwärts' || status.status === 'Überkauft' || status.status === 'Niedrig' ? 'bearish' : 'neutral';
            ...
            return `
                <div class="prediction-card">
                    ...
                    <div class="prediction-confidence status-${directionClass}">
                        ${formatPercentage(prediction.confidence, 1)} Konfidenz
                    </div>
        ...
        function formatDecimal(value, decimals = 2) {
            ...
            // Stelle sicher, dass wir immer die richtige Anzahl Dezimalstellen haben
            return num.toFixed(decimals);
        }

        function formatScore(value, maxValue) {
            if (value === null || value === undefined || isNaN(value)) return `0.0/${maxValue}`;
            const num = Number(value);
            if (num % 1 === 0) {
                // Ganze Zahl - zeige mit .0
                return `${num.toFixed(1)}/${maxValue}`;
            } else {
                // Dezimalzahl - formatiere sauber
                return `${num.toFixed(1)}/${maxValue}`;
            }
        }
        ...
        function formatCurrency(value, symbol = '$') {
            ...
            const num = Number(value);
            if (num >= 1e9) return `${symbol}${formatDecimal(num / 1e9, 2)}B`;
            if (num >= 1e6) return `${symbol}${formatDecimal(num / 1e6, 2)}M`;
            if (num >= 1e3) return `${symbol}${formatDecimal(num / 1e3, 2)}K`;
            return `${symbol}${formatDecimal(num, 2)}`;
        }
