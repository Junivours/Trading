...
# Liquidation Map API Endpoints
COINGLASS_LIQMAP_URL = "https://open-api.coinglass.com/public/v2/liquidation_chart"
...
def convert_to_py(obj):
    if isinstance(obj, np.ndarray):
        ...
        return [None if (isinstance(x, (float, np.floating)) and (np.isnan(x) or np.isinf(x))) else convert_to_py(x) for x in obj.tolist()]
...
class AdvancedTechnicalAnalyzer:
    @staticmethod
    def calculate_essential_indicators(ohlc_data):
        try:
            ...
            # ESSENTIAL TREND INDICATORS (reduced overlap)
            indicators['ema_20'] = talib.EMA(close, timeperiod=20)  # Short term trend
            indicators['ema_50'] = talib.EMA(close, timeperiod=50)  # Medium term trend
            indicators['sma_200'] = talib.SMA(close, timeperiod=200)  # Long term trend

            # MOMENTUM (only RSI - most reliable)
            indicators['rsi_14'] = talib.RSI(close, timeperiod=14)

            # TREND STRENGTH (ADX only - most reliable)
            ...
            # VOLUME CONFIRMATION (OBV only)
            indicators['obv'] = talib.OBV(close, volume)

            # MACD (trend momentum combination)
...
# Advanced Liquidation Map Engine
class LiquidationHeatmap:
    @staticmethod
    def fetch_liquidation_data(symbol="BTCUSDT", timeframe="1h"):
        ...
        try:
            ...
            # Method 1: Calculate liquidation clusters from Open Interest
            oi_data = LiquidationHeatmap.get_open_interest_data(symbol)
            if oi_data:
                liquidation_data.update(oi_data)
            
            # Method 2: Get funding rate as liquidation indicator
            funding_data = LiquidationHeatmap.get_funding_rate_data(symbol)
            if funding_data:
                liquidation_data['funding_rate'] = funding_data
            
            # Method 3: Calculate probable liquidation zones
            price_data = LiquidationHeatmap.get_recent_price_data(symbol)
            if price_data:
                liq_zones = LiquidationHeatmap.calculate_liquidation_zones(price_data)
                liquidation_data['liquidation_zones'] = liq_zones
    ...
    def get_open_interest_data(symbol):
        ...
        try:
            ...
            oi_url = f"{BINANCE_FUTURES_URL}/hist/openInterestHist"
            oi_params = {
                'symbol': symbol,
                'period': '1h',
                'limit': 24
            ...
            oi_hist = oi_response.json() if oi_response.status_code == 200 else []
            
            return {
                'current_oi': float(data['openInterest']),
                'oi_change_24h': LiquidationHeatmap.calculate_oi_change(oi_hist),
                'oi_trend': 'increasing' if len(oi_hist) > 1 and float(oi_hist[-1]['sumOpenInterest']) > float(oi_hist[-2]['sumOpenInterest']) else 'decreasing'
            }
    ...
    @staticmethod
    def get_funding_rate_data(symbol):
        """Get current funding rate - high funding = potential liquidations"""
        try:
            ...
            if funding_rate > 0.001:  # 0.1%
                sentiment = 'EXTREMELY_BULLISH_RISK'
            elif funding_rate > 0.0005:  # 0.05%
                sentiment = 'BULLISH_RISK'
            elif funding_rate < -0.001:
                sentiment = 'EXTREMELY_BEARISH_RISK'
    ...
    @staticmethod
    def get_recent_price_data(symbol):
        """Get recent price data for liquidation zone calculation"""
        try:
    ...
    @staticmethod
    def calculate_liquidation_zones(price_data):
        """Calculate probable liquidation zones based on price action"""
        try:
            prices = price_data['prices']
            current_price = price_data['current_price']
            
            # Calculate support/resistance levels (common liquidation zones)
            highs = [p['high'] for p in prices[-50:]]  # Last 50 candles
            lows = [p['low'] for p in prices[-50:]]
            
            # Find significant levels using volume profile approximation
            price_ranges = []
            ...
            # Sort by volume to find high-volume areas (liquidation magnets)
            sorted_by_volume = sorted(price_ranges, key=lambda x: x['volume'], reverse=True)
            top_volume_levels = sorted_by_volume[:10]  # Top 10 volume levels
            ...
            for level in top_volume_levels:
                if level['price'] < current_price:
                    ...
                    if 1 <= distance_percent <= 15:  # 1-15% below current price
                        liquidation_zones.append({
                            'price': level['price'],
                            'type': 'LONG_LIQUIDATION',
                            'distance_percent': distance_percent,
                            'volume_strength': level['volume'],
                            'risk_level': 'HIGH' if distance_percent < 5 else 'MEDIUM'
                        })
            ...
            for level in top_volume_levels:
                if level['price'] > current_price:
                    ...
                    if 1 <= distance_percent <= 15:  # 1-15% above current price
                        liquidation_zones.append({
                            'price': level['price'],
            ...
            liquidation_zones.sort(key=lambda x: x['distance_percent'])
            
            return liquidation_zones[:8]  # Return top 8 closest zones
            
        except Exception as e:
            logger.error(f"Error calculating liquidation zones: {str(e)}")
            return []
    ...
    def calculate_oi_change(oi_hist):
        ...
        try:
            if len(oi_hist) >= 2:
                ...
                previous = float(oi_hist[0]['sumOpenInterest'])
                return ((latest - previous) / previous) * 100
            return 0
        except:
            return 0

# Reduced Pattern Detection Engine (Essential Patterns Only)
class EssentialPatternDetector:
    @staticmethod
    def detect_key_patterns(ohlc_data):
        """Detect only the most reliable and non-overlapping patterns"""
        try:
            if len(ohlc_data) < 20:
                return {}
            ...
            # TIER 1: Most Reliable Single Candle Patterns
            ...
            # TIER 1: Most Reliable Two Candle Patterns
            ...
            # TIER 1: Most Reliable Three Candle Patterns
            ...
            patterns['total_active_patterns'] = active_patterns
            
            # Signal strength based on pattern combination
            if patterns['engulfing_bullish'] or patterns['morning_star'] or patterns['hammer']:
                patterns['signal'] = 'BULLISH'
                patterns['signal_strength'] = 'HIGH' if active_patterns >= 2 else 'MEDIUM'
            elif patterns['engulfing_bearish'] or patterns['evening_star'] or patterns['shooting_star']:
                patterns['signal'] = 'BEARISH'
                patterns['signal_strength'] = 'HIGH' if active_patterns >= 2 else 'MEDIUM'
            else:
                patterns['signal'] = 'NEUTRAL'
                patterns['signal_strength'] = 'LOW'
...
class AdvancedMLPredictor:
    ...
    def _extract_comprehensive_features(indicators, patterns, price_data, volume_data):
        ...
        if len(recent_volumes) > 1:
            try:
                ...
                features['volume_spike'] = recent_volumes[-1] / avg_vol if avg_vol != 0 else 1
            except (ZeroDivisionError, ValueError):
        ...
        # Technical indicator features (with safe defaults)
        features['rsi'] = max(0, min(100, indicators.get('current_rsi_14', 50)))
    ...
    def _calculate_trend_strength(indicators):
        ...
        ema_20 = indicators.get('current_ema_20', 0) or 0
        ema_50 = indicators.get('current_ema_50', 0) or 0
        sma_200 = indicators.get('current_sma_200', 0) or 0
        
        # Handle None or invalid values
        if not all(isinstance(x, (int, float)) and not (math.isnan(x) if isinstance(x, float) else False) for x in [ema_20, ema_50, sma_200]):
            return 0.5
...
class AdvancedMarketAnalyzer:
    ...
    def _generate_trading_signals(indicators, patterns, ml_predictions, price_data):
        ...
        try:
            ...
            if ml_predictions:
                for timeframe, prediction in ml_predictions.items():
                    if prediction.get('direction') == 'BUY' and prediction.get('confidence', 0) > 70:
                        signals.append({
                            'type': 'BUY',
                            'source': f'ML-{timeframe}',
                            'strength': min(10, int(prediction.get('confidence', 0) / 10)),
                            'message': f'ML {timeframe}: {prediction.get("confidence", 0):.1f}% confident',
            ...
            # Pattern Signals
            bullish_patterns = ['engulfing_bullish', 'hammer', 'morning_star']
            bearish_patterns = ['engulfing_bearish', 'shooting_star', 'evening_star']
            ...
            for pattern in bullish_patterns:
                if patterns.get(pattern, False):
                    signals.append({
                        ...
                        'source': 'Pattern',
                        'strength': 6,
                        'message': f'Bullish Pattern: {pattern.replace("_", " ").title()}',
                        'confidence': 70
                    })
            ...
            for pattern in bearish_patterns:
                if patterns.get(pattern, False):
                    signals.append({
                        ...
                        'source': 'Pattern',
                        'strength': 6,
                        'message': f'Bearish Pattern: {pattern.replace("_", " ").title()}',
                        'confidence': 70
                    })
    ...
    def _calculate_trading_kpis(indicators, patterns, price_data, volume_data):
        ...
        try:
            ...
            if len(price_data) > 10:
                recent_prices = [p.get('close', 0) for p in price_data[-20:]]
                kpis['volatility'] = (np.std(recent_prices) / np.mean(recent_prices) * 100) if recent_prices else 0
            else:
                kpis['volatility'] = 0
            
            # Trend strength
            kpis['trend_strength'] = 0
            ema_20 = indicators.get('current_ema_20', 0)
            ema_50 = indicators.get('current_ema_50', 0)
            if ema_20 > ema_50:
                kpis['trend_strength'] = 70
            elif ema_20 < ema_50:
                kpis['trend_strength'] = 30
            else:
                kpis['trend_strength'] = 50
            
            # Volume trend
            if len(volume_data) > 5:
                recent_vol = volume_data[-5:]
                prev_vol = volume_data[-10:-5] if len(volume_data) >= 10 else volume_data[:-5]
                if prev_vol:
                    kpis['volume_trend'] = (np.mean(recent_vol) - np.mean(prev_vol)) / np.mean(prev_vol) * 100
                else:
                    kpis['volume_trend'] = 0
            else:
                kpis['volume_trend'] = 0
            ...
            # RSI momentum
            kpis['rsi_momentum'] = indicators.get('current_rsi_14', 50)
            
            # Support/Resistance levels
            if len(price_data) > 20:
                recent_prices = [p.get('close', 0) for p in price_data[-20:]]
                kpis['support_level'] = min(recent_prices)
                kpis['resistance_level'] = max(recent_prices)
            else:
                kpis['support_level'] = 0
                kpis['resistance_level'] = 0
    ...
    def _calculate_sentiment_score(signals, kpis, ml_predictions):
        ...
        try:
            ...
            buy_score = sum(s.get('strength', 0) for s in buy_signals)
            sell_score = sum(s.get('strength', 0) for s in sell_signals)
            
            total_score = buy_score - sell_score
            sentiment_data['trading_score'] = total_score
            
            if total_score > 10:
                sentiment_data['overall_sentiment'] = 'BULLISH'
                sentiment_data['recommended_action'] = 'BUY'
                sentiment_data['confidence'] = min(95, 60 + abs(total_score) * 2)
                sentiment_data['strength'] = min(10, 6 + abs(total_score) // 5)
            elif total_score < -10:
                sentiment_data['overall_sentiment'] = 'BEARISH'
                sentiment_data['recommended_action'] = 'SELL'
                sentiment_data['confidence'] = min(95, 60 + abs(total_score) * 2)
                sentiment_data['strength'] = min(10, 6 + abs(total_score) // 5)
            else:
                sentiment_data['overall_sentiment'] = 'NEUTRAL'
                sentiment_data['recommended_action'] = 'HOLD'
                sentiment_data['confidence'] = 50
                sentiment_data['strength'] = 5
    ...
    def _detect_market_state(indicators, price_data, volume_data):
        ...
        try:
            volatility = 0
            if len(price_data) > 10:
                recent_prices = [p.get('close', 0) for p in price_data[-10:]]
            ...
            logger.error(f"Error detecting market state: {str(e)}")
            return 'UNKNOWN'
...
def analyze_symbol():
    try:
        ...
        price_data = [{'close': c['close']} for c in ohlc_data]
        volume_data = [c['volume'] for c in ohlc_data]
        indicators = AdvancedTechnicalAnalyzer.calculate_essential_indicators(ohlc_data)
        patterns = EssentialPatternDetector.detect_key_patterns(ohlc_data)
        
        # Add Liquidation Map Data
        liquidation_map = LiquidationHeatmap.fetch_liquidation_data(symbol)
        ...
        response = {
            ...
            'liquidation_map': liquidation_map,  # NEW: Liquidation Heatmap
...
def get_liquidation_heatmap():
    """Dedicated endpoint for real-time liquidation heatmap data"""
    try:
        req = request.get_json()
        symbol = req.get('symbol', 'BTCUSDT')
        
        # Fetch comprehensive liquidation data
        liquidation_data = LiquidationHeatmap.fetch_liquidation_data(symbol)
        
        if liquidation_data:
            return jsonify(convert_to_py(liquidation_data))
        else:
            return jsonify({'error': 'Could not fetch liquidation data'}), 500
            
    except Exception as e:
        logger.error(f"Error in /api/liquidation: {str(e)}")
        return jsonify({'error': str(e)}), 500
...
<body>
    ...
    <script>
        ...
        function displayAnalysis(data) {
            ...
            const liquidationMap = createLiquidationMapCard(data.liquidation_map); // NEW: Liquidation Heatmap
            ...
            // Append to dashboard
            [priceCard, kpiOverview, liquidationMap, technicalIndicators, mlPredictions, 
        ...
        function createLiquidationMapCard(liquidationData) {
            const card = document.createElement('div');
            card.className = 'card analysis-card';
            ...
            const fundingRate = liquidationData.funding_rate || {};
            const liquidationZones = liquidationData.liquidation_zones || [];
            const currentOI = liquidationData.current_oi || 0;
            const oiTrend = liquidationData.oi_trend || 'neutral';
            
            // Generate liquidation zone bars
            const liqZoneHtml = liquidationZones.map(zone => {
                const isLong = zone.type === 'LONG_LIQUIDATION';
                const color = isLong ? '#ef4444' : '#10b981';
                const icon = isLong ? '📉' : '📈';
                
                return `
                    <div class="liquidation-zone" style="
                    ...
                    ">
                        <div style="display: flex; align-items: center; gap: 0.5rem;">
                            <span>${icon}</span>
                            <span style="font-size: 0.9rem; color: ${color};">${zone.type.replace('_', ' ')}</span>
                        </div>
                        <div style="text-align: right;">
                            <div style="font-weight: bold;">$${zone.price.toLocaleString()}</div>
                            <div style="font-size: 0.8rem; color: var(--text-secondary);">
                                ${zone.distance_percent.toFixed(1)}% | ${zone.risk_level}
                            </div>
                        </div>
                    </div>
                `;
            ...
            // Funding rate color
            const fundingColor = fundingRate.funding_rate > 0.0005 ? '#ef4444' : 
                                fundingRate.funding_rate < -0.0005 ? '#10b981' : '#f59e0b';
                                
            card.innerHTML = `
                <h3 class="card-title">
                    <i class="fas fa-fire"></i>
                    Liquidation Heatmap 🔥
                    <span class="live-indicator" style="background: #ef4444;">LIVE</span>
                </h3>
                
                <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 1rem; margin-bottom: 1rem;">
                    <div class="funding-rate-card" style="
                        background: linear-gradient(135deg, ${fundingColor}20, ${fundingColor}10);
                        border: 1px solid ${fundingColor}40;
                        border-radius: 8px;
                        padding: 1rem;
                        text-align: center;
                    ">
                        ...
                        <div style="font-size: 1.2rem; font-weight: bold; color: ${fundingColor};">
                            ${fundingRate.funding_rate_percent ? fundingRate.funding_rate_percent.toFixed(4) : '0.0000'}%
                        </div>
                        <div style="font-size: 0.7rem; color: var(--text-secondary); margin-top: 0.3rem;">
                            ${fundingRate.funding_sentiment || 'NEUTRAL'}
                        </div>
                    </div>
                    
                    <div class="open-interest-card" style="
                    ...
                    ">
                        ...
                        <div style="font-size: 0.7rem; color: var(--text-secondary); margin-top: 0.3rem;">
                            ${oiTrend.toUpperCase()} 
                            ${oiTrend === 'increasing' ? '📈' : oiTrend === 'decreasing' ? '📉' : '➡️'}
                        </div>
                    </div>
                </div>
                ...
                <div style="margin-top: 1rem;">
                    ...
                    <div class="liquidation-zones" style="max-height: 200px; overflow-y: auto;">
                        ${liqZoneHtml || '<div style="text-align: center; color: var(--text-secondary); padding: 1rem;">No liquidation zones detected</div>'}
                    </div>
                </div>
        ...
        function createTechnicalIndicatorsCard(indicators) {
            ...
            // Helper function to get current value from indicator (last value if array)
            ...
            const currentRSI = indicators.current_rsi_14 || getCurrentValue(indicators.rsi_14);
            const currentMACD = indicators.current_macd || getCurrentValue(indicators.macd);
            const currentStoch = indicators.current_stoch_k || getCurrentValue(indicators.stoch_k);
            const currentWilliams = indicators.current_williams_r || getCurrentValue(indicators.williams_r);
            const currentCCI = indicators.current_cci || getCurrentValue(indicators.cci);
            const currentADX = indicators.current_adx || getCurrentValue(indicators.adx);
            const currentATR = indicators.current_atr || getCurrentValue(indicators.atr);
            const currentOBV = indicators.current_obv || getCurrentValue(indicators.obv);
            ...
            card.innerHTML = `
                ...
                <div class="indicators-grid">
                    ${createIndicatorItem('RSI (14)', currentRSI, getRSIStatus(currentRSI), 'Relative Strength Index')}
                    ${createIndicatorItem('MACD', currentMACD, getMACDStatus(currentMACD, indicators.current_macd_signal), 'MACD Linie vs Signal')}
                    ${createIndicatorItem('Stochastic', currentStoch, getStochStatus(currentStoch), 'Stochastic Oscillator %K')}
                    ${createIndicatorItem('Williams %R', currentWilliams, getWilliamsStatus(currentWilliams), 'Williams Percent Range')}
                    ${createIndicatorItem('CCI', currentCCI, getCCIStatus(currentCCI), 'Commodity Channel Index')}
                    ${createIndicatorItem('ADX', currentADX, getADXStatus(currentADX), 'Average Directional Index')}
                    ${createIndicatorItem('ATR', currentATR, getATRStatus(currentATR), 'Average True Range')}
                    ${createIndicatorItem('OBV', currentOBV, getOBVStatus(currentOBV), 'On Balance Volume')}
                </div>
            `;
            return card;
        }
        ...
        function createIndicatorItem(name, value, status, description) {
            const formatValue = (val) => {
                ...
                // Für RSI, Stochastic, Williams %R: 1 Dezimalstelle
                if (name.includes('RSI') || name.includes('Stochastic') || name.includes('Williams')) {
                    return val.toFixed(1);
                }
                // Für MACD, ATR: 4 Dezimalstellen (meist kleine Werte)
                if (name.includes('MACD') || name.includes('ATR')) {
                    return val.toFixed(4);
                }
                // Für große Zahlen wie OBV: keine Dezimalstellen
                if (name.includes('OBV') && Math.abs(val) > 1000) {
            ...
            return `
                <div class="indicator-card ${status.class}">
                    ...
                    <div class="indicator-value">${formatValue(value)}</div>
