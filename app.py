...
<body>
    ...
    <script>
        ...
        function createTechnicalIndicatorsCard(indicators) {
            ...
            const trendContent = `
                <div class="indicators-grid">
                    ${createIndicatorItem('EMA 20', indicators.ema_20, getEMAStatus(indicators.ema_20, indicators.current_close), 'Exponential Moving Average 20')}
                    ${createIndicatorItem('SMA 50', indicators.sma_50, getSMAStatus(indicators.sma_50, indicators.current_close), 'Simple Moving Average 50')}
                    ${createIndicatorItem('ADX', indicators.adx, getADXStatus(indicators.adx), 'Average Directional Index')}
                    ${createIndicatorItem('Parabolic SAR', indicators.sar, getSARStatus(indicators.sar, indicators.current_close), 'Parabolic Stop and Reverse')}
                </div>
            `;
            ...
            const momentumContent = `
                <div class="indicators-grid">
                    ...
                    ${createIndicatorItem('MACD Signal', indicators.macd_signal, getMACDSignalStatus(indicators.macd_signal), 'MACD Signal Line')}
                    ${createIndicatorItem('ATR', indicators.atr, getATRStatus(indicators.atr), 'Average True Range')}
                    ${createIndicatorItem('BB Position', calculateBBPosition(indicators), getBBPositionStatus(indicators), 'Bollinger Bands Position')}
                </div>
            `;
            ...
            const volumeContent = `
                <div class="indicators-grid">
                    ...
                    ${createIndicatorItem('A/D Line', indicators.ad, getADStatus(indicators.ad), 'Accumulation/Distribution Line')}
                    ${createIndicatorItem('ADOSC', indicators.adosc, getADOSCStatus(indicators.adosc), 'A/D Oscillator')}
                    ${createIndicatorItem('Volume Trend', 'Neutral', {class: 'neutral', text: 'NORMAL'}, 'Volume Analysis')}
                </div>
            `;
        ...
        function createIndicatorItem(name, value, status, description) {
            return `
                <div class="indicator-card ${status.class}">
                    ...
                    <div class="indicator-value">${typeof value === 'number' ? formatDecimal(value, 2) : value}</div>
