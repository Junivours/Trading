...
if __name__ == '__main__':
    ...
    # CRITICAL: Production-ready configuration
    app.run(
        host='0.0.0.0', 
        port=port, 
        debug=False, 
...
def analyze_symbol():
    ...
    try:
        ...
        logger.info(f"🚀 Analysis started for {symbol} ({interval})")
        
        # Validate symbol format
        if not (symbol.endswith('USDT') or symbol.endswith('BUSD') or symbol.endswith('BTC')):
            raise Exception(f"Invalid symbol format: {symbol}")
        ...
        kline_data = fetch_binance_data(symbol, interval, 200)
        ...
        indicators = AdvancedTechnicalAnalyzer.calculate_all_indicators(kline_data)
        
        # Detect patterns
        logger.info("Detecting candlestick patterns")
        patterns = AdvancedPatternDetector.detect_all_patterns(kline_data)
        
        # Generate ML predictions
        logger.info("Generating ML predictions")
        volumes = [d['volume'] for d in kline_data]
        ml_predictions = AdvancedMLPredictor.calculate_predictions(indicators, patterns, kline_data, volumes)
        
        # Comprehensive market analysis
        logger.info("Analyzing market sentiment and generating KPIs")
        market_analysis = AdvancedMarketAnalyzer.analyze_comprehensive_market(indicators, patterns, ml_predictions, kline_data, volumes)
        ...
        response = {
            ...
            'open_24h': ticker_data['open_price'],
            'indicators': {
                # Core indicators
                'rsi_14': indicators.get('current_rsi_14', 50),
                'rsi_7': indicators.get('current_rsi_7', 50),
                'rsi_21': indicators.get('current_rsi_21', 50),
                'macd': indicators.get('current_macd', 0),
                'macd_signal': indicators.get('current_macd_signal', 0),
                'macd_histogram': indicators.get('current_macd_histogram', 0),
                'stoch_k': indicators.get('current_stoch_k', 50),
                'stoch_d': indicators.get('current_stoch_d', 50),
                'williams_r': indicators.get('current_williams_r', -50),
                'cci': indicators.get('current_cci', 0),
                'adx': indicators.get('current_adx', 20),
                'atr': indicators.get('current_atr', 0),
                'sar': indicators.get('current_sar', current_price),
                # Moving averages
                'sma_5': indicators.get('current_sma_5', current_price),
                'sma_10': indicators.get('current_sma_10', current_price),
                'sma_20': indicators.get('current_sma_20', current_price),
                'sma_50': indicators.get('current_sma_50', current_price),
                'sma_200': indicators.get('current_sma_200', current_price),
                'ema_5': indicators.get('current_ema_5', current_price),
                'ema_10': indicators.get('current_ema_10', current_price),
                'ema_20': indicators.get('current_ema_20', current_price),
                'ema_50': indicators.get('current_ema_50', current_price),
                'ema_200': indicators.get('current_ema_200', current_price),
                # Bollinger Bands
                ...
                # Volume indicators
                'obv': indicators.get('current_obv', 0),
                'ad': indicators.get('current_ad', 0),
                'adosc': indicators.get('current_adosc', 0)
            },
            'patterns': patterns,
            'ml_predictions': ml_predictions,
            'market_analysis': market_analysis,
            'data_quality': {
                'candles_analyzed': len(kline_data),
                'indicators_calculated': len([k for k in indicators.keys() if k.startswith('current_')]),
                'patterns_detected': sum(1 for detected in patterns.values() if detected),
                'data_completeness': min(100, (len(kline_data) / 200) * 100)
            }
        }
        
        logger.info(f"✅ Analysis completed for {symbol} ({interval}) - Sentiment: {market_analysis['overall_sentiment']}")
        ...
        return jsonify({
            'error': error_msg,
            'timestamp': datetime.now().isoformat(),
            'symbol': data.get('symbol', 'UNKNOWN') if 'data' in locals() else 'UNKNOWN'
        }), 400
...
def get_watchlist():
    """Get popular trading pairs for watchlist"""
    watchlist = [
...
class AdvancedTechnicalAnalyzer:
    ...
    def calculate_all_indicators(ohlc_data):
        ...
        try:
            ...
            # Convert to float arrays for talib
...
class AdvancedPatternDetector:
    ...
    def detect_all_patterns(ohlc_data):
        """Detect comprehensive candlestick patterns using TA-Lib"""
        try:
            if len(ohlc_data) < 10:
                return {}
...
class AdvancedMLPredictor:
    ...
    def calculate_predictions(indicators, patterns, price_data, volume_data):
        """Advanced ML predictions with multiple algorithms"""
        try:
...
class AdvancedMarketAnalyzer:
    ...
    def _generate_trading_signals(indicators, patterns, ml_predictions, price_data):
        ...
        if macd > macd_signal and macd_hist > 0:
            signals.append({
                ...
                'reason': 'MACD above signal line with positive histogram',
                'strength': 8,
                'confidence': 85,
            ...
            signals.append({
                ...
                'reason': 'MACD below signal line with negative histogram',
                'strength': 8,
                'confidence': 85,
                'timeframe': 'Medium Term',
                'category': 'Trend'
        ...
        if ema_20 > ema_50 > sma_200:
            signals.append({
                ...
                'reason': 'Bullish MA alignment: EMA20 > EMA50 > SMA200',
                'strength': 8,
                'confidence': 88,
                'timeframe': 'Long Term',
                'category': 'Trend'
            ...
            signals.append({
                'type': 'SELL',
                'indicator': 'Moving Average Alignment',
                'reason': 'Bearish MA alignment: EMA20 < EMA50 < SMA200',
                'strength': 8,
                'confidence': 88,
                'timeframe': 'Long Term',
                'category': 'Trend'
        ...
        if len(price_data) >= 10:
            recent_volumes = [p['volume'] for p in price_data[-10:]]
            ...
            if current_volume > avg_volume * 2:
                ...
                signals.append({
                    ...
                    'reason': f'Volume spike {current_volume/avg_volume:.1f}x average with price {"+movement" if price_change > 0 else "decline"}',
                    'strength': 8,
                    'confidence': 82,
                    'timeframe': 'Short Term',
                    'category': 'Volume'
        ...
        bullish_patterns = ['engulfing_bullish', 'hammer', 'morning_star', 'three_white_soldiers', 'piercing_line', 'dragonfly_doji']
        bearish_patterns = ['engulfing_bearish', 'shooting_star', 'evening_star', 'three_black_crows', 'dark_cloud_cover', 'gravestone_doji']
        
        for pattern in bullish_patterns:
            if patterns.get(pattern, False):
                strength = 9 if pattern in ['engulfing_bullish', 'morning_star', 'three_white_soldiers'] else 7
                signals.append({
                    'type': 'STRONG BUY' if strength > 8 else 'BUY',
                    'indicator': 'Candlestick Pattern',
                    'reason': f'{pattern.replace("_", " ").title()} pattern detected',
                    'strength': strength,
                    'confidence': 85 if strength > 8 else 75,
                    'timeframe': 'Short Term',
                    'category': 'Pattern'
                })
        ...
        for pattern in bearish_patterns:
            if patterns.get(pattern, False):
                strength = 9 if pattern in ['engulfing_bearish', 'evening_star', 'three_black_crows'] else 7
                signals.append({
                    'type': 'STRONG SELL' if strength > 8 else 'SELL',
                    'indicator': 'Candlestick Pattern',
                    'reason': f'{pattern.replace("_", " ").title()} pattern detected',
                    'strength': strength,
                    'confidence': 85 if strength > 8 else 75,
        ...
        ml_bullish = sum(1 for pred in ml_predictions.values() if pred.get('direction') == 'BUY')
        ml_bearish = sum(1 for pred in ml_predictions.values() if pred.get('direction') == 'SELL')
        
        if ml_bullish >= 3:
            avg_confidence = np.mean([pred.get('confidence', 50) for pred in ml_predictions.values() if pred.get('direction') == 'BUY'])
            signals.append({
                'type': 'BUY',
                'indicator': 'ML Consensus',
                'reason': f'{ml_bullish}/5 ML models predict bullish movement',
                'strength': 8,
                'confidence': min(90, avg_confidence),
                'timeframe': 'Multi-Frame',
                'category': 'AI Prediction'
            })
        elif ml_bearish >= 3:
            avg_confidence = np.mean([pred.get('confidence', 50) for pred in ml_predictions.values() if pred.get('direction') == 'SELL'])
            signals.append({
                'type': 'SELL',
                'indicator': 'ML Consensus',
                'reason': f'{ml_bearish}/5 ML models predict bearish movement',
                'strength': 8,
                'confidence': min(90, avg_confidence),
                'timeframe': 'Multi-Frame',
                'category': 'AI Prediction'
    ...
    def _calculate_trading_kpis(indicators, patterns, price_data, volume_data):
        """Calculate comprehensive trading KPIs"""
        kpis = {}
        ...
        try:
            ...
            if len(price_data) >= 20:
                recent_prices = [p['close'] for p in price_data[-20:]]
                kpis['volatility_20d'] = (np.std(recent_prices) / np.mean(recent_prices)) * 100
                
                daily_returns = [(recent_prices[i] - recent_prices[i-1]) / recent_prices[i-1] for i in range(1, len(recent_prices))]
                kpis['avg_daily_return'] = np.mean(daily_returns) * 100
                kpis['sharpe_ratio'] = (np.mean(daily_returns) / np.std(daily_returns)) * np.sqrt(252) if np.std(daily_returns) != 0 else 0
            ...
            kpis['distance_from_sma200'] = ((current_price - sma_200) / sma_200) * 100 if sma_200 != 0 else 0
            
            # Momentum KPIs
            rsi = indicators.get('current_rsi_14', 50)
            kpis['momentum_score'] = 0
            if 30 <= rsi <= 70:
                kpis['momentum_score'] += 1  # Neutral RSI
            if indicators.get('current_macd', 0) > indicators.get('current_macd_signal', 0):
                kpis['momentum_score'] += 1  # Bullish MACD
            if indicators.get('current_adx', 0) > 25:
                kpis['momentum_score'] += 1  # Strong trend
            
            # Volume KPIs
            if len(volume_data) >= 20:
                recent_volumes = volume_data[-20:]
                kpis['volume_sma_ratio'] = recent_volumes[-1] / np.mean(recent_volumes[:-1])
                kpis['volume_trend'] = 'INCREASING' if recent_volumes[-1] > recent_volumes[-5] else 'DECREASING'
            
            # Pattern Recognition KPIs
            total_patterns = len(patterns)
            detected_patterns = sum(1 for detected in patterns.values() if detected)
            kpis['pattern_detection_rate'] = (detected_patterns / total_patterns) * 100 if total_patterns > 0 else 0
            
            bullish_patterns = ['engulfing_bullish', 'hammer', 'morning_star', 'three_white_soldiers', 'piercing_line']
            bearish_patterns = ['engulfing_bearish', 'shooting_star', 'evening_star', 'three_black_crows', 'dark_cloud_cover']
            
            kpis['bullish_pattern_count'] = sum(1 for p in bullish_patterns if patterns.get(p, False))
            kpis['bearish_pattern_count'] = sum(1 for p in bearish_patterns if patterns.get(p, False))
            kpis['pattern_sentiment'] = kpis['bullish_pattern_count'] - kpis['bearish_pattern_count']
            
            # Support/Resistance KPIs
            if len(price_data) >= 50:
                highs = [p['high'] for p in price_data[-50:]]
                lows = [p['low'] for p in price_data[-50:]]
                kpis['resistance_level'] = max(highs)
                kpis['support_level'] = min(lows)
                kpis['distance_to_resistance'] = ((kpis['resistance_level'] - current_price) / current_price) * 100
                kpis['distance_to_support'] = ((current_price - kpis['support_level']) / current_price) * 100
            
            # Risk KPIs
            atr = indicators.get('current_atr', 0)
            kpis['risk_reward_ratio'] = atr / current_price * 100 if current_price != 0 else 0
            
            bb_upper = indicators.get('current_bb_upper', current_price)
            bb_lower = indicators.get('current_bb_lower', current_price)
            if bb_upper != bb_lower:
                kpis['bb_position'] = (current_price - bb_lower) / (bb_upper - bb_lower)
                kpis['bb_squeeze'] = (bb_upper - bb_lower) / current_price < 0.1
            else:
                kpis['bb_position'] = 0.5
                kpis['bb_squeeze'] = True
            
            # Market Efficiency KPIs
            williams_r = indicators.get('current_williams_r', -50)
            stoch_k = indicators.get('current_stoch_k', 50)
            
            kpis['market_efficiency'] = 0
            if -80 <= williams_r <= -20:
                kpis['market_efficiency'] += 1
            if 20 <= stoch_k <= 80:
                kpis['market_efficiency'] += 1
            if 30 <= rsi <= 70:
                kpis['market_efficiency'] += 1
            
            kpis['market_efficiency'] = (kpis['market_efficiency'] / 3) * 100
    ...
    def _calculate_sentiment_score(signals, kpis, ml_predictions):
        """Calculate overall sentiment and trading score"""
        total_score = 0
        confidence_sum = 0
        signal_count = 0
        ...
        for signal in signals:
            weight = (signal['strength'] / 10) * (signal['confidence'] / 100)
            if 'BUY' in signal['type']:
                multiplier = 2 if 'STRONG' in signal['type'] else 1
        ...
        # Add KPI-based scoring
        trend_strength = kpis.get('trend_strength', 0)
        momentum_score = kpis.get('momentum_score', 0)
        pattern_sentiment = kpis.get('pattern_sentiment', 0)
        
        kpi_score = (trend_strength / 4) + (momentum_score / 3) + (pattern_sentiment / 5)
        ...
        for pred in ml_predictions.values():
            if pred.get('direction') == 'BUY':
                ml_consensus += pred.get('confidence', 50) / 100
            elif pred.get('direction') == 'SELL':
                ml_consensus -= pred.get('confidence', 50) / 100
        
        total_score += ml_consensus / len(ml_predictions)
        ...
        avg_confidence = confidence_sum / signal_count if signal_count > 0 else 50
        final_confidence = min(95, max(30, avg_confidence * (1 + abs(total_score) / 10)))
        
        # Calculate trading score (0-100)
        trading_score = min(100, max(0, 50 + (total_score * 10)))
        
        # Calculate strength (1-10)
        strength = min(10, max(1, 5 + abs(total_score)))
    ...
    def _detect_market_state(indicators, price_data, volume_data):
        ...
        try:
            ...
            if adx > 40 and abs(ema_20 - ema_50) / current_price > 0.02:
...
def set_cached_data(key, data):
    """Set data in cache with cleanup"""
    global api_cache
...
def fetch_binance_data(symbol, interval="1h", limit=200):
    """Fetch comprehensive Binance data"""
...
def cleanup_cache_service():
    """Background service to cleanup expired cache entries"""
    while True:
        try:
